...
const TooltipContent = forwardRef(({ className, sideOffset = 4, ...props }, ref) => (
  <TooltipPrimitive.Content
    ...
    className={cn(
      'z-50 overflow-hidden rounded-md border border-[#333333] bg-[#1F2023] px-3 py-1.5 text-sm text-white shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
      className
    )}
    {...props}
  />
...
const PromptInputBox = forwardRef(({ onSend = () => {}, isLoading = false, placeholder = 'Type your message here...', className }, ref) => {
              ...
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    ...
                    transition={{ type: 'spring', stiffness: 260, damping: 25 }}
                  >
                    <Code2 className={cn('w-4 h-4', showCanvas ? 'text-[#F97316]' : 'text-inherit')} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showCanvas && (
